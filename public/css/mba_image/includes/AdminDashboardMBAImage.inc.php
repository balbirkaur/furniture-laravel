...
$items = MBAImage::getAllArrayOfObjects();
$sorteditems = MBAImage::getAllArrayOfObjects(array('sorted' => true));
$thisitem = new MBAImage(array('id' => $itemid));
$categories = MBAImageCategory::getAllListingArrayOfObjects();
...
<link rel="stylesheet" href="<?= $SITECLASS->urlNormalize('modules/mba_image/css/adminmbaimage.css') ?>" type="text/css" />
...
<script type="text/javascript" src="<?= $SITECLASS->urlNormalize('modules/mba_image/scripts/adminmbaimage.js') ?>"></script>
<style type="text/css">
	#preview img {
		margin: 5px;
	}
...
<section>
	<div class="admin_envelope">
		...
		<div id="general_formpart_enclosure" <?= $display == 'general' ? '' : ' class="hidden"' ?>>
			<form name="form_adminmbaimage" action="<?= $SITECLASS->urlNormalize('admin/dashboard') ?>" method="post" enctype="multipart/form-data">
				...
				<div class="item">
					<label>
						...
						<?
						$tit = 'Category';
						$msg = 'Select the category or cagegories to associate this entry with. If no categories are displayed below you must use the category management page to define one or more.';
						?>
						<img class="site_help_message pointer" message="<?= $msg ?>" title="<?= $tit ?>" />
					</label>
					<?
					if(sizeof($categories) > 5) {
						$colsize = (sizeof($categories) / 3) + 1;
					} else {
						$colsize = 6;
					}
					$colcount = 1;
					$catids = $thisitem->CategoryIDs;
					?>
					<div class="clearfix">
						<div class="grid_33">
							<? foreach($categories as $c) { ?>
							<input type="checkbox" name="categoryids[]" value="<?= $c->ID ?>" <?= in_array($c->ID, $catids) ? ' checked="checked"' : '' ?> /><?= $c->Title ?><br />
							<? if($colcount++ % $colsize == 0) { ?>
						</div>
						<div class="grid_33">
							<? } ?>
							<? } ?>
				...
				<textarea id="tmce_htmlcontent" name="htmlcontent"><?= $thisitem->HTMLContent ?></textarea>
		</div>
		*/ ?>
		<div class="item">
			<label>
				...
				<?
						...
						?>
				<img class="site_help_message pointer" message="<?= $msg ?>" title="<?= $tit ?>" />
				<input type="file" id='resortimages' name="resortimages[]" multiple><br>
				<input type="button" id="submitimages" value='Upload'>
				<div id='preview'></div>
		</div>
		<div class="item">
			<label>
				...
				<?
						...
						?>
				<img class="site_help_message pointer" message="<?= $msg ?>" title="<?= $tit ?>" />
				<a href="javascript:" onclick="adminmbaimage.addMBAImageImage();">[add/edit item image]</a>
			</label>
			<input type="text" name="imageurl" value="<?= $thisitem->ImageURL ?>" onchange="adminmbaimage.updateMBAImageImage({'url':this.value});" />
			<div id="adminmbaimage_image_envelope" class="clearfix<?= $thisitem->ImageURL ? '' : ' hidden' ?>">
				...
				<select name="imagecrop" onchange="adminmbaimage.changeMBAImageImageCrop({'which':'#adminmbaimage_image_display'});">
