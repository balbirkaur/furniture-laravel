<?
class AdminDashboardMBAImageFormHandler {
		...
		public function formHandler($in = array()) {
			...
			$posted = isset($form_incoming) && $form_incoming == 'AdminMBAImage' && isset($form_action) ? true : false;
			if($posted) {
				switch($form_action) {
					case 'Update':
						$i = new MBAImage(array('id' => $form_itemid));
						...
						$i->ImageCrop = isset($form_resortimages) ? $form_resortimages : null;
						$i->ActiveFlag = isset($form_activeflag) && $form_activeflag == 'true' ? 1 : 0;
						if($i->save()) {
							$form_categoryids = isset($form_categoryids) && is_array($form_categoryids) ? $form_categoryids : array($form_categoryids);
							$i->saveCategoriesByIDs(array('ids' => $form_categoryids));
